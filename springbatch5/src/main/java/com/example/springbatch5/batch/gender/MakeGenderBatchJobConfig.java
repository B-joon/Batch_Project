package com.example.springbatch5.batch.gender;
...
public class MakeGenderBatchJobConfig {
     ...
     * @return
     */
    @Bean(name = "makeGenderBatchJob")
    public Job makeGenderBatchJob(JobRepository jobRepository) {
        return new JobBuilder("makeGenderBatchJob", jobRepository)
                .start(makeGenderBatchStep(jobRepository))
                .build();
    }
    ...
    @Bean
    @JobScope
    public Step makeGenderBatchStep(JobRepository jobRepository) {
        return new StepBuilder("makeGenderBatchStep", jobRepository)
                // 데이터를 가공하기 위한 개수 지정
                // db에 데이터가 1000개가 있고 chunk를 100으로 지정하면 100개 씩 총 10번 동작함.
                // <FollowerVO, GenderVO> FollowerVO : 호출 데이터 VO, GenderVO : 가공 후 DB 저장 데이터 VO
                .<FollowerVO, GenderVO>chunk(2, transactionManager)
                // 데이터 호출을 위한 메소드 지정
    ...
    public MyBatisCursorItemReader<FollowerVO> addGenderBatchItemReader(@Value("#{JobParameters['create_at']}") String date) {
        ...
        reader.setQueryId("com.example.springbatch5.persistence.mapper.FollowerMapper.genderData");
    ...
    public MyBatisBatchItemWriter<GenderVO> addGenderBatchItemWriter() {
        ...
        writer.setStatementId("com.example.springbatch5.persistence.mapper.BatchMapper.setGender");
